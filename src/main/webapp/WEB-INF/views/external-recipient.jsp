...
<link rel="stylesheet"
	...
	<div class="container">
		...
		<form:form action="${pageContext.request.contextPath}/transfer/externalrecipient/save" method="POST">
		<label>Name</label>
		...
		<label>Phone</label>
		<input type="number" value="${recipient.phone}" name="phone" id="phone" required/><br>
			...
			<c:if test="${not empty recipientList}">
				<c:forEach items="${recipientList}" var="list">
					...
					<tr>
						...
						<td><a href="${pageContext.request.contextPath}/transfer/delete?recipientName=${list.name}">Delete</a></td>
